...
a=args.ssml||args.m||args.markup;if(a)argstack.push('-m');argstack.push('--path='+eSpeakDir,String(text));if(args.log&&self.console)console.log('executing espeak '+argstack.join(' '));if(typeof callback!=='function')callback=null;if(!callback&&typeof args.callback=='function')callback=args.callback;if(hasWorker){sendWorkerJob({'job':'speak','args':argstack},id,function(stream){resolveUtterance(stream,id,args.rawdata,args.volume,args.pan||0,callback);});}
...
var mobileAudioUnlocked=false;function mobileAudioUnlocker(event){event.returnValue=true;if(mobileAudioUnlocked)return;if(AudioAPI){if(!audioContext)audioContext=new AudioAPI();var buffer=audioContext.createBuffer(1,1,22050),source=audioContext.createBufferSource();audioContext.buffer=buffer;audioContext.connect(audioContext.destination);(source.noteOn||source.start)(0);}
mobileAudioUnlocked=true;self.removeEventListener('touchstart',mobileAudioUnlocker);}
loadCore();resolveAudioApi();window.addEventListener('unload',unloadHandler,false);if(isMobile&&typeof self.ontouchstart!==undefined)self.addEventListener('touchstart',mobileAudioUnlocker,false);return{'speak':speak,'speakMultipart':speakMultipart,'loadConfig':loadConfig,'loadVoice':loadVoice,'loadCustomConfig':loadCustomConfig,'setDefaultVoice':setDefaultVoice,'getDefaultVoice':getDefaultVoice,'setVolume':setVolume,'getVolume':getVolume,'play':playSound,'isConfigLoaded':isConfigLoaded,'isVoiceLoaded':isVoiceLoaded,'resetQueue':resetQueue,'canPlay':canPlaybackWav,'stop':stopSound,'setFilter':setFilter,'getRunMode':getRunMode,'restartWithInstance':restartWithInstance,'getAudioAnalyser':getAudioAnalyser};})();